...
def stall_update_Fred(cnt_stalls, perceived_stall, dur_stalls, t_dur_stalls, play, flg_qoe, flg_qoe_init):
    ...
    return cnt_stalls, perceived_stall, dur_stalls, t_dur_stalls, flg_qoe, flg_qoe_init


# Compute QoE Value
def compute_QoE(cnt_stalls, t_dur_stalls, requests):
    sum_ql = sum_ql_sq = 0
    ...
    # If the video was displayed
        ...
        avg_quality = sum_ql/float(cnt_requests)

        # compute quality standard deviation
        sig_quality = math.sqrt(abs((sum_ql_sq-2*avg_quality*sum_ql+float(cnt_requests)*avg_quality**2)/float(cnt_requests)))

        try:
            qoe = max(5.67*(avg_quality/float(q_max))-6.72*(sig_quality/float(q_max))+0.17-4.95*((7.0/8.0)*max((math.log(float(cnt_stalls)/(float(config.T)/1000))/6)+1, 0)+(1.0/8.0)*min(avg_stall, 15)/15.0), 0)

        except (ValueError, ZeroDivisionError):
            qoe = max(5.67*(avg_quality/float(q_max))-6.72*(sig_quality/float(q_max))+0.17-4.95*(1.0/8.0)*min(avg_stall, 15)/15.0, 0)

        print sum_ql, sum_ql_sq, avg_quality, avg_stall, sig_quality, cnt_stalls, cnt_requests, t_dur_stalls, qoe
