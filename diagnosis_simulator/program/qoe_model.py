...
# Update QoE value
def update_QoE(cnt_milisegments, cnt_stalls, perceived_stall, dur_stalls, t_dur_stalls, sum_ql, sum_ql_sq, play, flg_qoe, flg_qoe_init, flg_start_QoE, init_t, requests, dataset, user, t):
    idx = 0
    ...
    # compute quality statistics
        ...
        if flg_start_QoE:
            ...
            vp_tiles = get_vp_tiles(client.get_longitude(dataset, user, t-t_dur_stalls-init_t))
            ...
            """print idx
            print requests[idx]['bitrate'], requests[idx-1]['bitrate']
            print requests"""

            if idx >= config.Binit/1000.0 and requests[idx]['bitrate'] <= requests[idx-1]['bitrate'] and requests[idx-1]['request_time'] != 0:
                flg_start_QoE = True
                vp_tiles = get_vp_tiles(client.get_longitude(dataset, user, t-t_dur_stalls-init_t))
                ql = get_vp_quality(requests, vp_tiles, t, t_dur_stalls+init_t)
                sum_ql += ql
